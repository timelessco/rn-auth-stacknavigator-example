...
const RegisterScreen: React.FC<RegisterProps> = (props) => {
  ...
  const onError = (errors) => {
    if (errors.email) {
      ...
      setShowToast({
        showToast: true,
        toastMessage: errorData.name?.message || "Invalid Name",
        toastIcon: null,
      });
    }
  };
  ...
  }, [errorData.email?.message, errorData.password?.message, errorData.name?.message]);
  ...
  }, [toastDetails.showToast, toastDetails.toastMessage]);
  ...
  return (
    <SafeAreaView style={{ flex: 1, alignItems: "center", backgroundColor: "white", position: "relative" }}>
      ...
      <Animated.View entering={FadeInDown} style={styles.headerWrapper}>
        <Text style={{ fontWeight: "700", fontSize: 24, textAlign: "center" }}>Hello!</Text>
        <Text style={{ fontWeight: "500", fontSize: 16, textAlign: "center", color: "gray", paddingTop: 12 }}>Please sign up to get started</Text>
      </Animated.View>
      <Animated.View entering={FadeIn} style={styles.formWrapper}>
        <TextInput
          style={[tw.style("px-2 py-3 rounded-lg bg-transparent my-1 mx-4 border-2 border-gray-200"), errorData.name ? tw.style("border-red-100") : {}]}
          {...register("name", {
            minLength: { value: 3, message: "Name should have minimum 3 characters" },
          })}
          placeholder='name'
        ...
        <TextInput
          ...
          onSubmitEditing={handleSubmit(registerUser, onError)}
        />
      </Animated.View>
      <Animated.View entering={FadeInDown} style={styles.buttonsWrapper}>
        <Pressable onPress={handleSubmit(registerUser, onError)} style={({ pressed }) => [styles.primaryButtonStyle, pressed ? styles.pressedPrimaryButtonStyle : {}]}>
